export const Store = {
  ...
  saveNewInstrument: function(inst) {
    console.log(inst);
    if (
      this.state.instrumentList.filter((n) => n.modelName !== inst.modelName)
        .length
    ) {
      this.state.instrumentList.push({
        modelName: inst.modelName,
        imageSrc: inst.instImageLink,
        type: inst.instType,
        desc: inst.desc,
        listPrice: inst.instPrice,
      });
    }
  },

  removeInstrument: function(modelName) {
    this.state.instrumentList = this.state.instrumentList.filter(
      (n) => n.modelName !== modelName
    );
  },
};
